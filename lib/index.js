...
function getConfig() {
    ...
    try {
        if (existsSync(configPath)) {
            ...
            return Object.assign(Object.assign(Object.assign({}, defaultConfig), config), { branches: Object.assign(Object.assign({}, defaultConfig.branches), config.branches) });
        }
        else {
...
async function handleExit(code, error) {
    if (!isExiting) {
        isExiting = true;
        if (cleanup.length > 0) {
            if (error) {
    ...
    for (const item of cleanup.reverse()) {
        try {
            if (item.type === 'stash') {
                ...
                if ((_a = item.data) === null || _a === void 0 ? void 0 : _a.tag) {
...
async function main() {
    try {
        ...
        if (config.verbose)
            console.log(chalk.cyan('ℹ'), chalk.white('verbose logging enabled'));
            ...
            try {
                ...
                if (tags.all.length > 0) {
                    ...
                    if (match && match[1]) {
                        ...
                        if (config.verbose)
                            console.log(chalk.cyan('ℹ'), chalk.white(`Found existing preid: ${existingPreId}`));
                    }
                }
                ...
                if (config.verbose)
                    console.error(chalk.yellow('⚠'), chalk.white('Unable to check latest tag for preid.'), error);
            }
            if (interrupted)
                return;
        ...
        if (config.stashChanges) {
            try {
                ...
                stashingSpinner === null || stashingSpinner === void 0 ? void 0 : stashingSpinner.error();
        ...
        if (config.stashChanges) {
            try {
                stashingSpinner === null || stashingSpinner === void 0 ? void 0 : stashingSpinner.update({ text: 'restoring stashed changes' });
                await git.stash(['pop']);
                cleanup = cleanup.filter(item => item.type !== 'stash');
                stashingSpinner === null || stashingSpinner === void 0 ? void 0 : stashingSpinner.success();
            }
            catch (error) {
                stashingSpinner === null || stashingSpinner === void 0 ? void 0 : stashingSpinner.error();
                versionSpinner.error();
        ...
        try {
            ...
            pushSpinner === null || pushSpinner === void 0 ? void 0 : pushSpinner.error();
            await handleExit(1, error);
            return;
        }
        if (!interrupted) {
...
async function setupGithubWorkflows() {
    ...
    try {
        for (var _d = true, workflows_1 = __asyncValues(workflows), workflows_1_1; workflows_1_1 = await workflows_1.next(), _a = workflows_1_1.done, !_a; _d = true) {
        ...
        try {
            if (!_d && !_a && (_b = workflows_1.return)) await _b.call(workflows_1);
        }
